    ...
    return (
        <div>
            <div className="flex items-center justify-between" >
                ...
                <Button className="cursor-pointer" size={'sm'} onClick={() => onReload(testCases[0]?.repoId)} ><RefreshCcw className="h-3 w-3 mr-1" /> Refresh</Button>
            </div>
            <div className="border rounded-md mt-3">
                ...
                <div className="p-4 flex items-center justify-between bg-gray-100">
                    ...
                    <Button disabled={selectedTestCases?.length == 0} className="cursor-pointer"> <Play className="h-4 w-4 mr-2" /> Run Selected</Button>
                </div>
            </div>
        </div>
    )
